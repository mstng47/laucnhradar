...
// Up to 8 main items each fire a deep-dive call at once (see the Promise.all
// in summarize.mjs), so a burst 429/529 from the API is expected, not
// exceptional - without a retry, whichever entries land on the wrong side of
// that burst lose their deep dive silently and permanently (the result gets
// written to Supabase as-is). One retry after a short pause covers the
// transient case without turning a real failure into a slow one.
...
async function createWithRetry(params) {
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.warn(`Deep dive call hit ${err.status}, retrying in ${RETRY_DELAY_MS}ms...`);
